...
| `--allow-foreign-layout-mismatch` |  | Repair from a diff whose nodes disagree on which inherited relations are foreign. Off by default. |  |
...
## Inheritance trees with foreign children

When the diffed table has foreign children, repair writes each row to the heap
relation recorded in the diff (`_spock_metadata_.storage_relation`) using
`INSERT INTO <relation> ... ON CONFLICT (<pk>) DO UPDATE` for changed and
missing rows and `DELETE FROM ONLY <relation>` for rows the source lacks. A row
the target node lacks is inserted into the relation of the same name the source
node holds it in, or into the parent when the target has no such heap relation.
Nothing is ever written to a foreign relation. Every heap relation in the tree
must have a primary key so that upserts can use `ON CONFLICT`.
`--fix-nulls` is not supported for these tables.

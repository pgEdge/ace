...
`UPDATE ONLY`, `DELETE FROM ONLY`, and `INSERT INTO <relation>`. A row the
target node lacks is inserted into the relation of the same name the source
node holds it in, or into the parent when the target has no such heap
relation. Nothing is ever written to a foreign relation. Every heap relation in
the tree must have a primary key so that upserts can use `ON CONFLICT`.
